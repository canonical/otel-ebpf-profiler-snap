...
# Update the snap to the latest upstream version (otelcol-specific: filters release tags)
...
update source_repo:
  ...
  full_version="$full_version" yq -i '.parts.ocb["source-tag"] = "v" + strenv(full_version)' "$snapcraft_file"
